...
def Ianshow_Permissions(dx):
    ...
    for i in p:
        method=""
        for j in p[i]:
            method+=(Ianshow_Path( dx.get_vm(), j ))+'\n'
	permission[i]=method
    return permission

def Ianshow_Path(vm, path):
  cm = vm.get_class_manager()
  stri=''
  ...
  if isinstance(path, PathVar):
    dst_class_name, dst_method_name, dst_descriptor =  path.get_dst( cm )
    info_var = path.get_var_info()
    stri= "%s %s (0x%x) ---> %s->%s%s" % (path.get_access_flag(),
                                          info_var,
                                          path.get_idx(),
                                          dst_class_name,
                                          dst_method_name,
    ...
    if path.get_access_flag() == TAINTED_PACKAGE_CALL:
      src_class_name, src_method_name, src_descriptor =  path.get_src( cm )
      dst_class_name, dst_method_name, dst_descriptor =  path.get_dst( cm )
      stri="%d %s->%s%s (0x%x) ---> %s->%s%s" % (path.get_access_flag(),
                                                  src_class_name,
                                                  src_method_name,
                                                  src_descriptor,
                                                  path.get_idx(),
      ...
      src_class_name, src_method_name, src_descriptor =  path.get_src( cm )
      stri="%d %s->%s%s (0x%x)" % (path.get_access_flag(),
                                    src_class_name,
                                    src_method_name,
                                    src_descriptor,
                                    path.get_idx())
