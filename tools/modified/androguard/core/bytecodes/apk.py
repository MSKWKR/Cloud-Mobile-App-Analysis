...
class APK(object):
    ...
    def __init__(self, filename, raw=False, mode="r", magic_file=None, zipmodule=ZIPMODULE):
        ...
        for i in self.zip.namelist():
            if i == "AndroidManifest.xml":
                ...
                # Enhanced encoding fix for AXMLPrinter output
                parse_success = False
                xml_buff = None

                def clean_malformed_attributes(xml_string):
                    """
                    Remove malformed XML attributes that have no name (e.g., ="value")
                    This happens when AXMLPrinter cannot retrieve attribute names from StringBlock
                    """
                    # Pattern: newline + optional whitespace + = + quote + value + quote + newline
                    # This matches standalone ="value" attributes on their own lines
                    xml_string = re.sub(r'\n\s*=[\"\']([^\"\']*)[\"\']', '', xml_string)
                    return xml_string

                try:
                    xml_buff = self.axml[i].get_buff()

                    # Check if buffer is empty
                    if not xml_buff or len(xml_buff) == 0:
                        print("[ERROR] AXMLPrinter returned empty buffer for AndroidManifest.xml")
                        self.xml[i] = None
                    else:
                        # Try different encoding strategies
                        if isinstance(xml_buff, bytes):
                            # Strategy 1: Try multiple encodings for bytes
                            for encoding in ['utf-8', 'latin-1', 'cp1252', 'iso-8859-1']:
                                try:
                                    xml_str = xml_buff.decode(encoding, errors='ignore')
                                    # Remove null bytes and control characters (keep \n, \r, \t)
                                    xml_str = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F]', '', xml_str)
                                    # Remove malformed attributes (="value" without attribute name)
                                    xml_str = clean_malformed_attributes(xml_str)
                                    self.xml[i] = minidom.parseString(xml_str.encode('utf-8'))
                                    parse_success = True
                                    print("[INFO] Successfully parsed AndroidManifest.xml using encoding: " + encoding)
                                    break
                                except Exception as e:
                                    continue
                        else:
                            # Strategy 2: Already a string, clean control characters
                            xml_str = xml_buff
                            ...
                            try:
                                self.xml[i] = minidom.parseString(xml_str)
                                parse_success = True
                                print("[INFO] Successfully parsed AndroidManifest.xml (string mode)")
                            except Exception as e:
                                # Try encoding to bytes and back
                                try:
                                    xml_str_cleaned = xml_str.encode('utf-8', errors='ignore').decode('utf-8')
                                    xml_str_cleaned = clean_malformed_attributes(xml_str_cleaned)
                                    self.xml[i] = minidom.parseString(xml_str_cleaned)
                                    parse_success = True
                                    print("[INFO] Successfully parsed AndroidManifest.xml after UTF-8 cleanup")
                                except:
                                    pass
                    ...
                    if xml_buff:
                        try:
                            preview = repr(xml_buff[:200]) if isinstance(xml_buff, bytes) else repr(xml_buff[:200])
                            print("  XML buffer preview (first 200 chars): " + preview)
                        except:
                            print("  Could not preview XML buffer")
                    self.xml[i] = None
    ...
    def get_elements(self, tag_name, attribute):
        ...
        l = []
        for i in self.xml:
            # Skip if XML parsing failed for this file
